...
func TestLogStreamer_detectNewLogs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			checkDetectNewLogsResult(
				t, streamer, newSteps, tt.currentLogs, initialStateCopy, tt.expectedNew, tt.expectedSteps,
			)
		})
	}
}

// checkDetectNewLogsResult asserts detectNewLogs' returned steps (count, indices,
// and entry deltas) and the streamer's updated line-count state.
func checkDetectNewLogsResult(
	t *testing.T,
	streamer *LogStreamer,
	newSteps []*StepLogs,
	currentLogs []*StepLogs,
	initialStateCopy map[int]int,
	expectedNew int,
	expectedSteps []int,
) {
	t.Helper()

	if len(newSteps) != expectedNew {
		t.Errorf("expected %d new steps, got %d", expectedNew, len(newSteps))
	}
	...
	if len(gotIndices) != len(expectedSteps) {
		t.Errorf("step indices: got %v, want %v", gotIndices, expectedSteps)
		return
	}
	...
	for _, newStep := range newSteps {
		originalStep := findStepByIndex(currentLogs, newStep.StepIndex)
		if originalStep == nil {
